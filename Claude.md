...
| 2026-07-09 | #5 | Єдиний Docker-контейнер: Next.js static export, FastAPI віддає його зі SPA-fallback; multistage `Dockerfile`, `docker-compose.yml` згорнуто до одного сервісу `app` | #13 |

### Відомі обмеження поточного оточення (оновлено 2026-07-09)

- ✅ **uv встановлено** (`~/.local/bin`), `uv sync --extra dev` та `uv run pytest` виконано успішно: 29/29 тести проходять (auth, documents, frontend-mount, security, health, app-boot).
- **Docker не встановлено локально** — єдиний multistage `backend/Dockerfile` та `docker-compose.yml` (сервіс `app`) написані за дизайном, але не перевірені запуском на цій машині. Логіка static-serving у FastAPI покрита тестами, а Next.js export збирається локально, але повний `docker compose up` (build+run образу) НЕ верифіковано.
